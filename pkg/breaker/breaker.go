...
// ========================================================================
// 【重点学习】熔断器模式（Circuit Breaker Pattern）
// ========================================================================
...
// 📝 简历亮点：
// - 理解熔断器的三种状态和转换条件
// - 熔断器参数调优（失败阈值、超时时间、半开请求数）
// - 与限流、降级的区别
//
// 🔥 面试高频：
// Q: 熔断和限流的区别？
// A: 限流是主动控制请求速率，保护自己；
//    熔断是被动响应下游故障，快速失败，防止级联故障
//
// Q: 熔断器的关键参数有哪些？
// A: 1) 失败阈值（连续失败多少次触发熔断）
//    2) 熔断时长（Open 状态持续多久）
//    3) 半开状态允许的请求数
//    4) 成功阈值（半开状态多少次成功后恢复）
//
// 面试高频问题（补充）：
// Q: 什么是“雪崩效应”？熔断器如何防止雪崩？
// A: 雪崩效应指微服务调用链中，某个基础服务不可用，导致上游服务线程阻塞、资源耗尽，最终整个系统瘫痪。
//    熔断器通过快速失败（Fail Fast）机制，当检测到下游服务异常时，直接返回错误，释放线程资源，
//    从而隔离故障，防止故障扩散。
//
// Q: 熔断器在 Half-Open 状态下是怎么工作的？
// A: 当熔断时间结束后，熔断器进入 Half-Open 状态。
//    此时允许少量请求（如 1 个）通过去访问下游服务。
//    - 如果请求成功，说明下游恢复，熔断器关闭 (Closed)。
//    - 如果请求失败，说明下游仍不可用，熔断器重新打开 (Open)。
// ========================================================================
