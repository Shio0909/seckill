 ...
 *
 * 核心知识点：
 * 1. Viper 库的使用：支持多种格式 (JSON, YAML, TOML) 和多种来源 (文件, 环境变量, 远程配置中心)。
 * 2. 结构体映射：使用 mapstructure 标签将配置文件映射到 Go 结构体。
 * 3. 热更新机制：利用 fsnotify 监听配置文件变化，实现不重启服务更新配置。
 * 4. 单例模式：使用 sync.Once 确保配置只被初始化一次。
 * 5. 读写锁：使用 sync.RWMutex 保证并发读取配置的安全性。
 *
 * 面试高频问题：
 * Q1: 为什么需要配置中心？Viper 如何实现热更新？
 * A1: 配置中心用于集中管理微服务配置，实现动态调整。Viper 通过 fsnotify 监听文件系统事件 (Write/Create/Remove)，
 *     当检测到文件变化时，重新读取并解析配置文件，更新内存中的配置对象。
 *
 * Q2: 在高并发场景下，如何安全地读取动态更新的配置？
 * A2: 使用读写锁 (sync.RWMutex)。读取配置时加读锁 (RLock)，更新配置时加写锁 (Lock)。
 *     或者使用 atomic.Value 存储配置指针，实现无锁读取 (Copy-On-Write 思路)。
 *
 * Q3: 环境变量和配置文件的优先级是怎样的？
 * A3: Viper 中，显式设置 > 命令行参数 > 环境变量 > 配置文件 > 默认值。
 *     这允许在容器化部署 (Docker/K8s) 时通过环境变量覆盖默认配置。
 */
package config
